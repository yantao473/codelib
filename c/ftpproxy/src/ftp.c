...
int dopasv(ftp_t *x, char *command, char *par)
{
    ...
    // bind port 20 for sae pasv mode listen
	ch->isock = bind_to_port(ch->inside.ipnum, 20);
...
int dologin(ftp_t *x)
{
	...
	if (*x->config->ctp != 0) {
        ...
        //TODO
        char *server_ip = hashtab_get("common70");
        if(server_ip != NULL){
            fprintf (stderr, "CLI >>>: %s, %s\n", "Get ip from hashtab:", server_ip);
            copy_string(x->server.name, server_ip, sizeof(*server_ip));
            syslog(LOG_NOTICE, "NOTICE: use hashtab server:%s\n", server_ip);
        }else{
            fprintf (stderr, "CLI >>>: %s, %s\n", "Get ip from default:", server_ip);
            syslog(LOG_NOTICE, "NOTICE: use default server:%s\n", x->config->server);
